...
func _build_shell(w: float, h: float, d: float, wall_mat: Material, roof_mat: Material, door_mat: Material, has_windows: bool = true) -> void:
	...
	# Mái
	var roof_h := 0.7
	_add_box(Vector3(w + 0.4, roof_h, d + 0.4), Vector3(0, h + roof_h * 0.35, 0), roof_mat)
	# Cửa chính
...
func _build_sawmill() -> void:
	...
	for i in 3:
		var z_off := -1.0 + i * 0.5
		_add_box(Vector3(1.2, 0.25, 0.25), Vector3(-2.0, 0.12 + i * 0.25, z_off), log_mat)
	# Bàn gỗ phía trước
	_add_box(Vector3(1.0, 0.1, 0.5), Vector3(0, 0.7, 2.2), _mat(Color(0.4, 0.25, 0.12)))
...
func _build_church() -> void:
	...
	# Cửa vòm (đắp nổi trên cửa)
	_add_box(Vector3(1.2, 0.3, 0.1), Vector3(0, 1.6, 2.26), wall_mat)
	# Tháp chuông phía trước
	var tower_mat := _mat(Color(0.82, 0.78, 0.68))
	_add_box(Vector3(1.5, 4.5, 1.5), Vector3(0, 2.25, 2.5), tower_mat)
	# Mái tháp nhọn
	_add_box(Vector3(1.8, 1.2, 1.8), Vector3(0, 5.3, 2.5), roof_mat)
	# Thập tự trên đỉnh tháp
	var cross_mat := _mat(Color(0.85, 0.75, 0.2), 0.4)
	_add_box(Vector3(0.08, 0.6, 0.08), Vector3(0, 6.3, 2.5), cross_mat)
	_add_box(Vector3(0.3, 0.08, 0.08), Vector3(0, 6.2, 2.5), cross_mat)
	# Chuông — vòng tròn tối trong tháp
	var bell_mat := _mat(Color(0.6, 0.45, 0.1), 0.5)
	_add_box(Vector3(0.5, 0.5, 0.5), Vector3(0, 3.5, 2.5), bell_mat)
	# Cửa sổ kính màu 2 bên hông
...
func _build_hospital() -> void:
	...
	# Dấu thập đỏ trên mái
	var red_mat := _mat(Color(0.85, 0.1, 0.1), 0.3)
	_add_box(Vector3(0.15, 0.6, 0.15), Vector3(0, 3.2, 0), red_mat)
	_add_box(Vector3(0.5, 0.15, 0.15), Vector3(0, 3.0, 0), red_mat)
	# Dấu thập đỏ trên tường phía trước
	_add_box(Vector3(0.1, 0.5, 0.08), Vector3(2.0, 1.8, 1.76), red_mat)
	_add_box(Vector3(0.35, 0.1, 0.08), Vector3(2.0, 1.8, 1.76), red_mat)
	# Bảng hiệu "bệnh viện" — box dài trên cửa
...
func _build_school() -> void:
	...
	_add_box(Vector3(0.8, 1.0, 0.8), Vector3(0, 3.3, 0), _mat(Color(0.7, 0.55, 0.25)))
	# Mái tháp
	_add_box(Vector3(1.0, 0.4, 1.0), Vector3(0, 4.0, 0), roof_mat)
	# Chuông
	_add_box(Vector3(0.3, 0.3, 0.3), Vector3(0, 3.5, 0), _mat(Color(0.6, 0.45, 0.1), 0.5))
	# Cột cờ trước trường
	var pole_mat := _mat(Color(0.3, 0.3, 0.3))
	_add_box(Vector3(0.08, 3.5, 0.08), Vector3(2.2, 1.75, 2.0), pole_mat)
	# Cờ — box đỏ nhỏ trên đỉnh cột
