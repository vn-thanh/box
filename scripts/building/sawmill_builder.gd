...
static func build(b: Building3D) -> void:
	...
	b._build_shell(3.5, 2.2, 3.0, wall_mat, roof_mat, door_mat, b._mat(Building3D.STONE_COL), true, 1.0, 0.4)
	...
	b._add_box(Vector3(0.4, 1.6, 0.4), Vector3(1.2, Building3D.FOUND_H + 3.0, -0.5), chimney_mat)
	b._add_box(Vector3(0.5, 0.1, 0.5), Vector3(1.2, Building3D.FOUND_H + 3.85, -0.5), chimney_mat)

	# Khúc gỗ chất bên hông
	var log_mat := b._mat(Color(0.50, 0.35, 0.18), 0.95)
	for i in 3:
		b._add_box(Vector3(1.0, 0.22, 0.22), Vector3(-2.2, Building3D.FOUND_H + 0.11 + i * 0.22, -1.0 + i * 0.5), log_mat)
